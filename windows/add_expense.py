"""Add Expense window — form for recording a new expense transaction.

Delegates date formatting to DateInput/DatePickerPopup and all field
validation to TransactionValidator before writing to the database.
"""
...
from windows.custom_components.theme import (
    SURFACE, TEXT, MUTED, INPUT, INPUT_HOVER, PINK, SOFT_PINK, FONT
)
...
class AddExpenseWindow(tk.Toplevel):
    """Form window for recording a new expense transaction.

    Collects amount, category, date (MM.DD.YYYY), and an optional note.
    Converts the date to YYYY-MM-DD before passing it to TransactionValidator
    and then to FinanceDatabase.
    ...
    def build_ui(self):
        """Construct the header and form fields: amount, category, date, and note."""
    ...
    def create_select(self, parent, row, label, variable):
        ...
        select = SelectBox(wrapper, variable, empty_text="No categories")
    ...
    def create_date_input(self, parent, row, label):
        ...
        date_input = DateInput(wrapper, self.var_date, today_bg=SOFT_PINK)
    ...
    def on_save(self):
        """Validate all fields and write the new expense record to the database."""
        try:
